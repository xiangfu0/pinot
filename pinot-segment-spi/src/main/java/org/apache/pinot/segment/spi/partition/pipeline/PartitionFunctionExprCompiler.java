...
public final class PartitionFunctionExprCompiler {
  ...
  // pinot-common (which provides CommonPartitionScalarFunctionResolver) is not yet on the classpath.
  // The JVM class-initialization guarantee makes this inherently thread-safe and single-init.
  private static final class FunctionResolverHolder {
    static final PartitionScalarFunctionResolver INSTANCE = loadFunctionResolver();
  }

  public static PartitionPipeline compile(String rawColumn, String functionExpr) {
    return compile(rawColumn, PartitionValueType.STRING, functionExpr, PartitionIntNormalizer.POSITIVE_MODULO);
  }
  ...
  public static PartitionPipeline compile(String rawColumn, PartitionValueType inputType, String functionExpr,
    ...
    CompileResult result = compile(rawColumn, inputType, root, steps, 0);
    ...
    return new PartitionPipeline(rawColumn, inputType, result._outputType, result._canonicalExpr,
  ...
  public static PartitionPipelineFunction compilePartitionFunction(String rawColumn, PartitionValueType inputType,
    ...
    return new PartitionPipelineFunction(compile(rawColumn, inputType, functionExpr, configuredPartitionIdNormalizer),
        numPartitions);
  }

  private static CompileResult compile(String rawColumn, PartitionValueType inputType, Node node,
    ...
    if (node instanceof IdentifierNode) {
      ...
      return CompileResult.dynamic(inputType, rawColumn, new ExecutableFunctionEvaluator.ColumnNode(rawColumn, 0));
    }
    ...
    for (int i = 0; i < functionNode._arguments.size(); i++) {
      CompileResult argument = compile(rawColumn, inputType, functionNode._arguments.get(i), steps, depth + 1);
    ...
    PartitionScalarFunctionResolver.ResolvedFunction resolvedFunction =
        getFunctionResolver().resolve(displayName, toResolverArguments(arguments));
  ...
  private static PartitionScalarFunctionResolver getFunctionResolver() {
    return FunctionResolverHolder.INSTANCE;
  }
  ...
  private static PartitionScalarFunctionResolver loadFunctionResolver() {
    ...
    for (PartitionScalarFunctionResolver r : ServiceLoader.load(PartitionScalarFunctionResolver.class)) {
      resolvers.add(r);
    }
  ...
  /**
   * Bridges a {@link PartitionScalarFunctionResolver.ResolvedFunction} to the
   * {@link ExecutableFunctionEvaluator.Invoker} contract required by {@link ExecutableFunctionEvaluator.FunctionNode}.
   */
