...
import org.apache.pinot.connector.spark.common.{Logging, PinotDataSourceReadOptions}
    ...
    if (readOptions.proxyEnabled && readOptions.grpcProxyUri.nonEmpty) {
      ...
      val (host, port) = proxyUri.split(":", 2) match {
        ...
        case Array(h) =>
          ...
          (h, defaultPort)
        case _ =>
          throw new IllegalArgumentException(s"Invalid grpcProxyUri: $proxyUri")
      }
      (host, port)
    } else {
  ...
  private val pinotServerBlockingStub = {
    ...
    val withProxyHeaders =
      if (readOptions.proxyEnabled && readOptions.grpcProxyUri.nonEmpty) {
        ...
        (readOptions.authHeader, readOptions.authToken) match {
          case (Some(header), Some(token)) =>
            val key = Metadata.Key.of(header.toLowerCase, Metadata.ASCII_STRING_MARSHALLER)
            md.put(key, token)
          case (Some(header), None) =>
            // no token
          case (None, Some(token)) =>
            val key = Metadata.Key.of("authorization", Metadata.ASCII_STRING_MARSHALLER)
            val value = if (token.startsWith("Bearer ")) token else s"Bearer $token"
            md.put(key, value)
          case _ =>
        }
