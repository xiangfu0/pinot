...
public class ThrottleOnCriticalHeapUsageExecutor extends DecoratorExecutorService {
   ...
   * The registrar should bind a gauge identified by its name (matching ServerGauge enum names)
   * to a {@link LongSupplier} that provides the metric value.
   *
   * This avoids introducing module dependencies from SPI to pinot-common's metrics classes.
   */
  public static void registerThrottleMetrics(ThrottleOnCriticalHeapUsageExecutor throttle,
      BiConsumer<String, LongSupplier> gaugeRegistrar) {
    gaugeRegistrar.accept("THROTTLE_EXECUTOR_QUEUE_SIZE", () -> (long) throttle.getQueueSize());
    gaugeRegistrar.accept("THROTTLE_EXECUTOR_QUEUED_TASKS_TOTAL", () -> (long) throttle.getQueuedTaskCount());
    gaugeRegistrar.accept("THROTTLE_EXECUTOR_PROCESSED_TASKS_TOTAL", () -> (long) throttle.getProcessedTaskCount());
    gaugeRegistrar.accept("THROTTLE_EXECUTOR_TIMED_OUT_TASKS_TOTAL", () -> (long) throttle.getTimedOutTaskCount());
