 ...
 *
 * <p>There are two mutually exclusive modes:
 * <ul>
 *   <li><b>Name mode</b> (legacy): configure {@code functionName} (and optionally {@code functionConfig}).
 *       Old and new nodes can all deserialize this format.</li>
 *   <li><b>Expression mode</b> (new): configure {@code functionExpr} (and optionally
 *       {@code partitionIdNormalizer}).  This mode produces JSON that omits {@code functionName}.
 *       <b>Known limitation:</b> expression-mode configs must only be written after <em>all</em>
 *       broker/server/controller nodes have been upgraded to a version that supports this feature.
 *       A node pre-dating expression mode will fail to deserialize such a config and will exclude
 *       the affected table from partition-aware routing.  No mixed-version safety gate is enforced
 *       at the controller write path; operators are responsible for ensuring the cluster is fully
 *       upgraded before enabling expression-mode partitioning.</li>
 * </ul>
 */
