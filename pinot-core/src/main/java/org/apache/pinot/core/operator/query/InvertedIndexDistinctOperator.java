...
public class InvertedIndexDistinctOperator extends BaseOperator<DistinctResultsBlock> {
   ...
   */
  static final NavigableMap<Integer, Integer> DEFAULT_COST_RATIO_BY_CARDINALITY;
  ...
  private boolean shouldUseInvertedIndex() {
    ...
    int costRatio = costRatioOverride != null ? costRatioOverride : getDefaultCostRatio(dictionaryCardinality);
  ...
  private DistinctResultsBlock executeInvertedIndexPath() {
    ...
    for (int dictId = 0; dictId < dictLength; dictId++) {
      ...
      if (filteredDocIds == null) {
        ...
        includeValue = ImmutableRoaringBitmap.intersects(docIds, filteredDocIds);
      }
