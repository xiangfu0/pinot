...
public class InvertedIndexDistinctCostHeuristicTest extends BaseQueriesTest {
  ...
  /**
   * With default costRatio=30 (for dictCard<=1000) and 10K docs, inverted index should be chosen when opted in.
   */
  @Test
  public void testDefaultCostRatioUsesInvertedIndex() {
    ...
    BaseOperator<DistinctResultsBlock> op = runDistinct(query);
    assertTrue(usedInvertedIndex(op), "Default costRatio=30 (dictCard=100): 100*30=3000 <= 10K → inverted index");
  }

  /**
   * Verify the default cost ratio boundary behavior:
   * dictCard=100, default costRatio=30 (from cardinality map) → threshold is 3000 docs.
  ...
  public void testDefaultRatioBoundary() {
    // Default costRatio=30 (dictCard=100 falls in <=1000 bucket): 100*30=3000 <= 10K docs → inverted index
    ...
    assertTrue(usedInvertedIndex(runDistinct(defaultQuery)),
        "Default costRatio=30: 100*30=3000 <= 10K → inverted index");
