...
/**
 * Validation-only tests for {@code TableConfigUtils.validate} that exercise the
 * "explicit dictionary required for raw forward index" rule across:
 ...
 *
 * <p>The core invariant under test: any column that uses an index requiring a dictionary
 * (inverted, FST, IFST) must have an explicit dictionary config when the forward index is raw.
 *
 * <p>This test does not build segments; segment-build/reload behavior is covered by
 * {@code TableIndexingTest} and the inverted/range index handler tests.
 */
public class RawForwardIndexValidationTest {
  ...
  @Test
  public void testDeltaDeltaCodecWithExplicitDictionaryFails() {
    // DELTADELTA is only applicable to raw forward indexes; once a dictionary is explicitly enabled,
    // the codec is no longer valid for the column.
    ObjectNode indexes = JsonUtils.newObjectNode();
    indexes.set("dictionary", JsonUtils.newObjectNode());
    ...
    TableConfig tc = new TableConfigBuilder(TableType.OFFLINE).setTableName(TABLE_NAME)
        .setFieldConfigList(List.of(fc))
        .build();
    assertInvalid(tc, "Compression codec: DELTADELTA is not applicable to dictionary encoded column");
  }
