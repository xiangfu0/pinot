...
public class InvertedIndexHandlerTest {
  ...
  public void testSegmentPreProcessorInvalidatesLegacyRawValueInvertedIndex()
    ...
    try {
      ...
      // packed inside columns.psf. The test forces v1 in `buildSimpleSegmentWithInvertedIndex` so the file must
      // be present here — fail fast rather than silently no-op if the precondition is violated.
      File invFile = new File(indexDir, "col.bitmap.inv");
      assertTrue(invFile.exists(),
          "Test setup invariant violated: expected v1 unpacked inverted index file at " + invFile);
