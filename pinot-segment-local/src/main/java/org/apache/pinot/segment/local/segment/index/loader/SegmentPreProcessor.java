...
public class SegmentPreProcessor implements AutoCloseable {
  ...
  public void process(@Nullable SegmentOperationsThrottlerSet segmentOperationsThrottlerSet)
    ...
    try (SegmentDirectory.Writer segmentWriter = _segmentDirectory.createWriter()) {
      ...
      //   (a) materializes a dictionary for a column that previously had none, when the new table config enables a
      //       dictionary-dependent secondary index (the ENABLE_DICTIONARY operation in ForwardIndexHandler);
      //   (b) updates the segment metadata's HAS_DICTIONARY / FORWARD_INDEX_ENCODING properties accordingly.
      // After this runs and metadata is reloaded, InvertedIndexHandler / RangeIndexHandler / FSTIndexHandler read
      // the refreshed metadata and build dict-id-based indexes on top of the new dictionary. If this order is
      // violated, downstream handlers fail with an IllegalStateException because the dictionary they require does
      // not yet exist.
  ...
  /**
   * Drops any inverted index file that uses the legacy raw-value embedded-dictionary format written by the now-deleted
   * {@code RawValueBitmapInvertedIndexCreator}. This must run before any other index handler so they don't try to
   * interpret the legacy buffer as the modern bitmap-inverted-index format.
   *
   * <p>Two outcomes after this method runs:
   * <ul>
   *   <li>If the user's table config still requests an inverted index on the column, {@link InvertedIndexHandler}
   *       (or {@link org.apache.pinot.segment.local.segment.index.loader.ForwardIndexHandler} if a dictionary
   *       must be created first) rebuilds it in the dict-id format on the same reload pass.</li>
   *   <li>If the user has removed the inverted index from the config, the index simply stays absent — the column
   *       still has its raw forward index, and queries scan it.</li>
   * </ul>
   */
  private void removeLegacyRawValueInvertedIndexes(SegmentDirectory.Writer segmentWriter)
    ...
    for (ColumnMetadata columnMetadata : segmentMetadata.getColumnMetadataMap().values()) {
      ...
      // Cheap pre-filter: the legacy creator only emitted raw-value inverted indexes for columns without a
      // dictionary, so any dict-encoded column with an inverted index is already in the standard format and we
      // can skip the buffer mmap + byte-pattern check entirely.
