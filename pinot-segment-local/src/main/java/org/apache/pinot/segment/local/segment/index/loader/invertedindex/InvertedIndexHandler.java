...
///
/// Every inverted index consumes dictionary IDs. If a column does not have a dictionary on disk,
/// [#createInvertedIndexForColumn] fails fast with an [IllegalStateException]. The
/// [org.apache.pinot.segment.local.segment.index.loader.SegmentPreProcessor] runs
/// [org.apache.pinot.segment.local.segment.index.loader.ForwardIndexHandler] first so its `ENABLE_DICTIONARY`
/// path can materialize the dictionary before this handler runs.
    ...
    if (!columnMetadata.hasDictionary()) {
      ...
      throw new IllegalStateException(
          "Cannot create inverted index for segment: " + segmentName + ", column: " + columnName
              + " — no dictionary present. ForwardIndexHandler must materialize the dictionary first.");
    }
