...
public class SegmentIndexCreationDriverImpl implements SegmentIndexCreationDriver {
  ...
  void buildIndexCreationInfo()
    ...
    for (String tsColumn : _config.getTimestampIndexConfigs().keySet()) {
      FieldSpec fieldSpec = _dataSchema.getFieldSpecFor(tsColumn);
      Object tsColumnDefaultNullValue = fieldSpec.getDefaultNullValue();
      for (TimestampIndexGranularity granularity : _config.getTimestampIndexConfigs().get(tsColumn)) {
        String columnNameWithGranularity =
            TimestampIndexGranularity.getColumnNameWithGranularity(tsColumn, granularity);
        ColumnStatistics columnProfile = _segmentStats.getColumnProfileFor(columnNameWithGranularity);
        Object defaultNullValue = tsColumnDefaultNullValue == null ? null
            : DateTimeFunctions.dateTrunc(granularity.toString(), (Long) tsColumnDefaultNullValue);
        _indexCreationInfoMap.put(columnNameWithGranularity,
            new ColumnIndexCreationInfo(columnProfile, true/*createDictionary*/, false, false/*isAutoGenerated*/,
                defaultNullValue));
      }
    }
